...
fn main() -> Result<()> {
    ...
    match &cli.command {
        Commands::Full => {
            ...
            for i in 0..100 {
                if db.insert(&Fact::new(
                    SubjectID(i), PredicateID((i % 10) as u8), ObjectID(i * 2), 0.95,
                )?).is_err() {
                    insert_ok = false;
                    break;
                }
            ...
            let tx_ok = txn.insert(Fact::new(SubjectID(999), PredicateID(9), ObjectID(999), 0.5)?).is_ok();
            ...
            check("Schema access", schema.len() > 0);
        ...
        Commands::Performance => {
            ...
            println!("  Insert throughput: {:.0} facts/sec", 1000.0 / insert_time.as_secs_f64());
            println!("  Query throughput:  {:.0} queries/sec", 1000.0 / query_time.as_secs_f64());
            println!("  Insert latency:   {:.2} us/fact", insert_time.as_micros() as f64 / 1000.0);
            println!("  Query latency:    {:.2} us/query", query_time.as_micros() as f64 / 1000.0);
            Ok(())
        }
        Commands::Storage => {
            ...
            println!("  Active facts:   {}", db.active_fact_count());
            println!("  Memory estimate: {:.2} MB", (db.fact_count() as f64 * 34.0) / 1_048_576.0);
            println!("  Fact size:       34 bytes (34 x {} = {} bytes)", 
                db.fact_count(), db.fact_count() * 34);
            Ok(())
        }
        Commands::Memory => {
            ...
            let fact_count = db.fact_count() as u64;
            let _memory_bytes = fact_count * 34u64;
            println!("  Allocated memory: {:.2} MB", (db.fact_count() as f64 * 34.0) / 1_048_576.0);
